...
describe ListingsController, type: :controller do
  ...
  describe 'GET #index' do
    it 'populates an array of listings created by current user to @listings' do
      current_user_listings = FactoryGirl.create_list(:listing, 3, user: @user)
      get :index
  ...
  describe 'POST #create' do
    context 'with valid attributes' do
      it 'saves the new listing to the database' do
        expect {
          post :create, listing: FactoryGirl.attributes_for(:listing, user: @user)
    ...
    context 'with invalid attributes' do
      it 'does not save the new listing in the database' do
        expect {
          post :create, listing: FactoryGirl.attributes_for(:invalid_listing, user: @user)
  ...
  describe 'GET #publish' do
    it 'makes the listing#open as true' do
      listing = FactoryGirl.create(:listing, user: @user)
      get :publish, listing_id: listing.id
  ...
  describe 'GET #unpublish' do
    it 'makes the listing#open as false' do
      listing = FactoryGirl.create(:opened_listing, user: @user)
      get :unpublish, listing_id: listing.id
  ...
  describe 'GET #search' do
    before do
      @tomakomai_listings =
        FactoryGirl.create_list(:opened_listing, 3, location: "北海道苫小牧市錦岡443")
      @tomakomai_listings_closed =
        FactoryGirl.create_list(:listing, 2, location: "北海道苫小牧市錦岡443")
    end

    it 'populates an array of listings location includes "苫小牧市"' do
      get :search,
        { 'search' => { 'location' => '苫小牧市', 'num_of_guest' => 1, 'where' => 'header' } }
